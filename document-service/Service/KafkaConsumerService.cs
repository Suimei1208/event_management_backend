...
namespace document_service.Service
{
    public class KafkaConsumerService : BackgroundService
    {
        ...
        private async Task HandleMessage(string eventid, DocumentDbContext ticketDbContext)
        {
            _logger.LogInformation("đã nhận thông điệp: " + eventid);
            var document = await ticketDbContext.Document.Where(x => x.eventId == int.Parse(eventid)).ToListAsync();
            if (document.Count == 0)
            {
                _logger.LogInformation("không tìm thấy eventid: " + eventid);
                return;
            }
            ticketDbContext.Document.RemoveRange(document);
